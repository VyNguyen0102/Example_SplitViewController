...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        splitViewController = UISplitViewController.init()
        if (NVDevice.isIpad()){
            // Remove this line to auto hide Master View controller
            splitViewController?.preferredDisplayMode = UISplitViewControllerDisplayMode.AllVisible
        } else {
            splitViewController?.preferredDisplayMode = UISplitViewControllerDisplayMode.PrimaryOverlay
        }
        
        let root = ViewControllerMaster.init()
        let detail = ViewControllerDetail.init()
        root.delegate = detail
        let rootNav = UINavigationController.init(rootViewController: root)
